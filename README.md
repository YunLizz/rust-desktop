...
| ✍️ 编辑器 | CodeMirror 6：行号、当前行高亮、行距/字距/字号调节、宋体/黑体/楷体/仿宋四种字体、两端对齐（Word 式）、首行缩进一键切换、查找替换、跨标签页撤销历史、全局搜索、Markdown 高亮开关（中文写作默认关闭，`#` `*` 原样显示） |
| 🗂 大纲 | 树形大纲（卷→章→节→要点）、从 AI 回复一键应用为大纲、一键从章节生成骨架、节点增删改 |
| 👥 人物 | 人物卡（外貌/性格/背景/目标）、角色定位、**关系网画布**（可拖拽布局） |
| 🗺 世界观 | 地点层级树（国家→城市→建筑）、设定描述 |
| ⏱ 时间线 | 事件按时间排序、关联章节 |
| 🎯 任务 | 任务链 + 看板（待办/进行中/已完成，卡片拖拽换列） |
| 📊 统计 | 总字数、今日新增、连续创作天数、近 30 天柱状图、各卷字数 |
| 🎲 本地起名机 | 纯本地算法：人物名（含复姓）/书名/地名，古风/仙侠/都市/西幻四种风格，自动排除已有角色名，不消耗 AI 额度 |
